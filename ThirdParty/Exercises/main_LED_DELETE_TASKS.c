...
int main(void)
{
  ...
  */
  ...
  xStatus = xTaskCreate(button_task, "BUTTON", 200, NULL, 4, &xTaskButton);
  configASSERT(xStatus == pdPASS);
...
static void led_1_task(void* pvParameters)
{
	...
	while(1)
	{
		...
		{
			vTaskSuspendAll(); 	// shutdown the scheduler
			nextTask = xTaskLed2;
			xTaskResumeAll();	// turn the context switch on again

			SEGGER_SYSVIEW_PrintfTarget("Delete LED 1 Task");
...
static void led_2_task(void* pvParameters)
{
	...
	while(1)
	{
		...
		if(xNotifyStatus == pdTRUE) // If status is false, simply rerun the task
		{
			...
			SEGGER_SYSVIEW_PrintfTarget("Delete LED 2 Task");
...
static void led_3_task(void* pvParameters)
{
	...
	while(1)
	{
		...
		if(xNotifyStatus == pdTRUE) // If status is false, simply rerun the task
		{
			...
			SEGGER_SYSVIEW_PrintfTarget("Delete LED 3 Task");
			HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0, GPIO_PIN_SET);
			vTaskDelete(xTaskButton);
			vTaskDelete(NULL); // Self Delete
		}
	}
}
...
static void button_task(void* pvParameters)
{
	...
	while(1)
	{
		btn_read = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_10);

		if(btn_read == GPIO_PIN_SET && btn_prev_read == GPIO_PIN_RESET)
		{
		...
		vTaskDelay(pdMS_TO_TICKS(10));  // 	Periodic task with vTaskDelau()
	}
}
